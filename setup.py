...
# Get all .py files for Cython compilation
package_files = get_py_files(package_path)

extensions = [
    Extension(os.path.splitext(os.path.relpath(f, directory_path))[0].replace(os.path.sep,'.'), [f]) for f in package_files
]

class CustomBuildExt(build_ext):
    """Custom build_ext command that removes .py files after building extensions."""
    def run(self):
        super().run()
        # Remove .py files here, ensuring they're not included in the final wheel
